...
const SHELL=[
  ...
  '/client-card-v201.js?v=20.1.0',
  '/schedule-client-v202.js?v=20.2.0',
  '/nutrition-off-v203.js?v=20.3.0',
  '/client-directions-v205.js?v=20.5.0',
  '/client-calendar-v206.js?v=20.6.0',
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(
    keys.filter(key=>key.startsWith('denisfit-v')&&key!==CACHE).map(key=>caches.delete(key))
...
self.addEventListener('push',event=>{
  ...
  event.waitUntil(self.registration.showNotification(data.title||'DenisFit',{
    body:data.body||'',tag:data.tag||'denisfit',icon:'/assets/icon-192.png',badge:'/assets/icon-192.png',data:{url:data.url||'/'}
  }));
});
