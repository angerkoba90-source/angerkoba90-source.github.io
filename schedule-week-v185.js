...
let renderToken=0,queued=false;
...
function withTimeout(p,ms=5000){return Promise.race([Promise.resolve(p),new Promise(resolve=>setTimeout(()=>resolve({__timeout:true}),ms))])}

async function countsForWeek(start){
  const s=await withTimeout(db.auth.getSession(),3500);if(s?.__timeout)return new Map();
  const uid=s?.data?.session?.user?.id;if(!uid)return new Map();
  const end=new Date(start);end.setDate(end.getDate()+7);
  const query=db.from('schedule_events').select('starts_at,status').eq('trainer_id',uid).gte('starts_at',new Date(`${dateKey(start)}T00:00:00`).toISOString()).lt('starts_at',new Date(`${dateKey(end)}T00:00:00`).toISOString());
  const q=await withTimeout(query,5000);if(q?.__timeout||q?.error)return new Map();
...
function queueInject(delay=0){if(queued)return;queued=true;setTimeout(injectWeek,delay)}
const observer=new MutationObserver(()=>{
  if(!document.getElementById('scheduleDate'))return;
  // Only initialize after the base schedule screen has been rendered.
  // Mutations inside our own week panel must never start another request.
  if(!document.getElementById('scheduleWeekOverview'))queueInject(0);
});
window.addEventListener('load',()=>{const main=document.getElementById('main');if(main)observer.observe(main,{childList:true,subtree:true});queueInject(200)});
document.addEventListener('change',e=>{if(e.target?.id==='scheduleDate'){renderToken++;queueInject(30)}});
})();
